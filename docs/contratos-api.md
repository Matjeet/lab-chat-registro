...
| Autenticación | La identidad la gestiona **Firebase Auth**. `POST /api/v1/registro` no exige cabecera de auth todavía (ver nota de seguridad en §3.1); en el resto de endpoints que se vayan añadiendo, esperar `Authorization: Bearer <idToken>`. |
...
Completa el alta de un usuario **que ya se autenticó con Firebase Auth** en el cliente (SDK
de Firebase: email/password, Google, etc.). Este servicio no gestiona contraseñas; solo
guarda el `uid` que Firebase asignó y el proveedor usado.

> ⚠️ **Nota de seguridad (estado actual):** `uid` y `proveedor` se toman **tal cual
> del cuerpo de la petición** — este endpoint todavía **no verifica** el ID token de Firebase.
> Cualquiera que conozca el contrato podría enviar un `uid` inventado. No usar en
> producción sin añadir esa verificación (filtro que valide `Authorization: Bearer <idToken>`
> con Firebase Admin SDK o un Resource Server, y derive estos dos valores del token, no del
> body). Ver `docs/testing-seguridad-owasp.md` (A07).
...
| Autenticación | Ninguna exigida por el endpoint hoy (ver nota de arriba) |

Cuerpo (el mínimo que ya envía `chat-frontend` hoy):
...
Cuerpo completo, con `proveedor` explícito (para un proveedor distinto de `password`):
...
| `uid` | string | sí | 1–128 caracteres. El UID que devuelve Firebase Auth tras autenticarse (`user.uid` del SDK). Único. |
| `proveedor` | string | **no** | Uno exacto de: `password`, `google.com`, `facebook.com`, `apple.com`, `github.com`, `twitter.com`, `phone`, `anonymous` (los `providerId` de Firebase Auth). **Si se omite, el servidor asume `"password"`.** Si se envía, debe ser uno de los valores soportados o se rechaza con 400. |

Se ignora cualquier campo extra del cuerpo (p. ej. un `password` residual de una integración
vieja: no se persiste ni se valida).
...
| `proveedor` | string | Proveedor de Firebase Auth usado en el alta. |
| `activo` | boolean | Siempre `true` en un alta nueva. |
| `createdAt` | string (ISO-8601) | Instante de creación en UTC. |

> El `uid` **no** se devuelve en la respuesta. No se devuelve cabecera `Location` en
> esta versión.
...
```json
{
  ...
  "errors": [
    ...
    { "field": "proveedor", "message": "proveedor no soportado" }
  ]
}
```

- `errors[].field` es el nombre del campo del cuerpo (`username`, `email`, `uid`,
  `proveedor`) — úsalo para marcar el input correspondiente en el formulario.
...
> `username`, el `email` o el `uid`, ni devuelve el valor enviado, para no permitir
> enumerar cuentas. El frontend debe mostrar un mensaje del tipo *"No se pudo completar el
> registro. Revisa los datos e inténtalo de nuevo."* y **no** intentar deducir qué campo falló.
...
curl -i -X POST http://localhost:8080/api/v1/registro \
  ...
  -d '{"username":"mateo","email":"mateo@example.com","uid":"aB3dEfGhIjKlMnOpQrStUvWxYz12","proveedor":"password"}'
```
...
3. **`uid` nunca vuelve en la respuesta.** No lo necesitas: ya lo tienes en el cliente
   tras autenticarte con el SDK de Firebase.
...
6. **Unicidad de `username`/`email`/`uid` no se puede pre-comprobar** (no hay endpoint
   para ello, también por el tema de enumeración). Se descubre al recibir el `409` del `POST`.
7. **Llama a este endpoint después de que Firebase confirme la autenticación**, no antes: el
   `uid` y el `proveedor` de este `POST` deben salir del resultado del SDK de Firebase
   (`userCredential.user.uid` / `providerId`), no inventarse ni pedirse al usuario.
8. **`proveedor` es opcional.** Si tu flujo solo usa email/password puedes omitirlo (el
   servidor asume `"password"`); mándalo explícito solo si soportas otros proveedores.
...
export type ProveedorAuth =
  | "password" | "google.com" | "facebook.com" | "apple.com"
...
export interface RegistroRequest {
  ...
  uid: string;           // 1–128, tal cual lo devuelve Firebase Auth (user.uid)
  proveedor?: ProveedorAuth; // opcional: si se omite, el servidor asume "password"
}
...
export interface FieldError {
  field: string;   // "username" | "email" | "uid" | "proveedor"
...
| Fecha | Cambio |
|---|---|
| 2026-09-12 | `POST /api/v1/registro` cambia de contraseña propia a Firebase Auth: `password` → `uid` (obligatorio) + `proveedor` (opcional, por defecto `password`). `RegistroResponse` gana `proveedor`. |
